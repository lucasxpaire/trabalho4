...
public class MercadoExemplo {
    ...
    public static void main(String[] args) {
        ...
        try (Connection connection = DriverManager.getConnection(url, user, password)) {
            MercadoDao dao = new MercadoDao(connection);
            // Exemplo de insercao
            Mercado novoObj = new Mercado();
            novoObj.setProduto(getRandomString());
            novoObj.setPreco(RANDOM.nextFloat() * 100);
            novoObj.setIdProduto(RANDOM.nextInt(10000));
            dao.insert(novoObj);

            // Exemplo de listagem
            List<Mercado> list = dao.getAll();
            for (Mercado obj : list) {
                System.out.print(obj.getProduto() + " | ");
                System.out.print(obj.getPreco() + " | ");
                System.out.print(obj.getIdProduto() + " | ");
                System.out.println();
            }
